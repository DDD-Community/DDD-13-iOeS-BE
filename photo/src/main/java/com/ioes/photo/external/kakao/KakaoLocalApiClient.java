 ...
 * 카카오 로컬 API 클라이언트.
 *
 * <p>coord2address 엔드포인트로 위·경도를 도로명/지번 주소로 역지오코딩하고,
 * search/address 엔드포인트로 검색어에 대한 주소 후보 목록을 조회한다.
...
public class KakaoLocalApiClient {
    ...
    public KakaoAddressSearch searchAddress(String query, int page, int size) {
        try {
            URI uri = UriComponentsBuilder.fromUriString(SEARCH_ADDRESS_URL)
                ...
                .build()
                .toUri();
            SearchAddressResponse response = httpClientUtils.get(uri, authHeader(), SearchAddressResponse.class);
            return KakaoAddressSearch.from(response);
        } catch (ResourceAccessException e) {
            log.error("{} 검색 API 타임아웃: {}", API_NAME, e.getMessage());
            throw new BusinessException(ExternalApiErrorCode.API_TIMEOUT, API_NAME + " API 응답 시간 초과");
        } catch (BusinessException e) {
            throw e;
        } catch (Exception e) {
            log.error("{} 검색 API 호출 실패: {}", API_NAME, e.getMessage());
            throw new BusinessException(ExternalApiErrorCode.API_CALL_FAILED, API_NAME + " API 호출 실패");
        }
    }
